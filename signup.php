	     ...
	     <div class="container">
	        <div class="row">
	           <div class="col-md-12">
	              <!-- Logo -->
	              <div class="logo">
	                 <h1><a href="index.php">INVENTARIS DENBEKANG V-44-02</a></h1>
	              </div>
	           </div>
	        </div>
	     </div>
		...
		if(isset($_POST["daftar"])){
			...
			if(empty($username) || empty($password) || empty($password2) || empty($nama) || empty($email)){
				...
				echo "<script>window.location=('http://127.0.0.1/inventaris/signup?pesan=$pesan')</script>";
					...
					$q1 = mysqli_query($koneksi,"insert into tb_user set nama='$nama',password='$password',username='$username',email='$email',level='user'");
					if($q1){
						echo "<script>window.alert('Pendaftaran Berhasil!')</script>";
						echo "<script>window.location=('http://127.0.0.1/inventaris/')</script>";
					}
				}
			}
		}
